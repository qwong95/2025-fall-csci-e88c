...
class UtilFunctionsPropertyTest
    ...
    test("satisfy the property: if (x, y, z) is a triple, then (y, x, z) is also a triple") {
    ...
    test("satisfy the property: if (x, y, z) is a triple, then (2y, 2x, 2z) is also a triple") {
        // generate from known triples up to 100
        forAll (triplesGen) { case (x, y, z) =>
